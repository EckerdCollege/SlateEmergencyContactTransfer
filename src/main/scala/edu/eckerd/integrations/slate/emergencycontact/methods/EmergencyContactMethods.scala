...
trait EmergencyContactMethods extends DBFunctions{
  ...
  /**
    * Partititon to groups takes all requests puts them through the parsingFunction toRows and then returns A tuple
  ...
  /**
    ...
    *         the database.
    */
  private def toRows(records: Seq[SlateEmergencyContactInfo])
            (implicit ec: ExecutionContext, db: JdbcProfile#Backend#Database)
  ...
  /**
    ...
    */
  private def changePriorities(listGroupedByBannerID: List[SlateEmergencyContactInfo])
  : List[SlateEmergencyContactInfo] = {
  ...
  private def dealWithNonCompliantRecords(list: List[SlateEmergencyContactInfo])
                                         (implicit ec: ExecutionContext): Future[Unit] = list.length match {
